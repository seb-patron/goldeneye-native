#!/usr/bin/env python3
"""One table that maps a node id to a position, so a route can be read.

WHY THIS EXISTS. Route paths name node ids and nothing maps an id back to a place. Validating a
route, or a wall set against a route, was impossible without it -- there was no way to check
walls against real routes for exactly this reason.
