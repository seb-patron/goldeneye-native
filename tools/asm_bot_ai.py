...
def main():
    ...
    for path in sorted(glob.glob(os.path.join(BOTS_DIR, "*.json"))):
        ...
        for arch in doc.get("archetypes", []):
            ...
            # kind is the JSON basename, so "personalities" and "skill_tiers". rstrip("s")
            # strips a CLASS of characters rather than one suffix, which turned
            # "personalities" into "personalitie" and would turn anything ending in a double
            # s into nonsense. These names end up as C symbols, so they are worth getting
            # right before engine code starts referring to them.
