...
| **Co-op** | **Beta** | Two to four players through a solo mission's own geometry, objectives and cutscenes. Both players walk, verified on macOS with each driven independently, they cannot shoot each other, one player going down no longer fails the mission for the team, and the dead player respawns. Still beta because the mission is authored around one Bond, so extra players are present rather than accounted for. [`COOP.md`](docs/COOP.md) |
