...
#
# usage: ./build_linux.sh {sdl|lib|port|app|all|run|env}
set -uo pipefail
...
sdl_flags() {
  ...
  if [ -f "$SDL/lib/pkgconfig/sdl2.pc" ]; then
    ...
    SDL_FROM="$SDL (built by './build_linux.sh sdl')"
    ...
    echo "       or run './build_linux.sh sdl' to build 2.30.9 from deps/ into $SDL" >&2
    return 1
  fi
  return 0
}
...
cmd_sdl() {
  ...
  local b="$HOME/.n64tvos/build-sdl2-linux"
...
build_port_layer() {
  ...
  for f in "$HERE"/Sources/ge_tvos_main.c "$HERE"/port/mac/ge_mac_main.c; do
    ...
    if "$CC" "${PORTFLAGS[@]}" -c "$f" -o "$o" 2>/dev/null; then pok=$((pok+1))
         ...
         echo "  linux port FAILED: $(basename "$f")"; fi
  done
  echo "linux port layer: $pok built, $pfail failed"
}
...
run_batch() {
  ...
  echo "$res" | grep '^FAIL ' | sed 's/^FAIL /  linux FAILED: /'
...
cmd_port() {
  if [ ! -d "$BUILD/obj" ] || [ -z "$(ls -A "$BUILD/obj" 2>/dev/null)" ]; then
    echo "no objects in $BUILD/obj -- run './build_linux.sh lib' once first"; return 1
  fi
  build_port_layer
}
...
# --------------------------------------------------------------------------- link
cmd_app() {
  [ -d "$BUILD/obj" ] || { echo "nothing built -- run './build_linux.sh lib'"; return 1; }
  sdl_flags || return 1
  ...
  local f
  for f in "${roots[@]}"; do
    [ -e "$f" ] || { echo "missing harness object $f -- run './build_linux.sh port'"; return 1; }
  done
  ...
  if [ -s "$manifest" ]; then
    ...
    echo "linux note: no object manifest; archiving every object found -- run './build_linux.sh lib'"
    for f in "$BUILD"/obj/*.o; do
      case "$f" in
        *"/port_ge_tvos_main.o"|*"/port_ge_mac_main.o") continue ;;
      esac
  ...
  echo "linux libge.a: $(du -h "$BUILD/libge.a" | cut -f1), ${#members[@]} members"
  ...
  if [ -x "$BIN" ]; then
    echo "linux binary: $BIN ($(du -h "$BIN" | cut -f1), $(file -b "$BIN" | cut -d, -f1-2))"
  else
...
cmd_run() {
  [ -x "$BIN" ] || { echo "no binary -- run './build_linux.sh all'"; return 1; }
