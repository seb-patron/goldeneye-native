...
A route-following bot on Bunker 1 produced `spd=0.000` with a non-zero move offset -- every unit
of it sideways -- while its heading only showed Bond's idle sway. The bot had been sidestepping
at full deflection for its entire run. Injection was not the problem: with the bot off the walk
block never executes, with it on it runs every frame.
...
Two things about that gate are worth flagging for whoever next reads this code, since both are
easy to miss and both cost time here. First, `canTurnTank` gating the *stick* does not mean
sideways movement is unreachable on 1.x: lines `6056-6064` call `bondviewUpdateSpeedSideways`
from `digitalStepLeft`/`digitalStepRight` -- the C-buttons and D-pad, set at `5659-5663` -- and
they run *before* the `canTurnTank` gate, unconditionally. Grepping only reads of `analogStrafe`
misses this; the writes to `speedsideways` itself are the ones that matter. Second, `6069` sits
inside an `else` whose `if` is `g_PlayerIsInTank == 1` at `5955` -- read in isolation, the `else`
