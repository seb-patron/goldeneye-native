...
| `GETV_BASETILE=0` | Restore the old unit-0 tile choice, which takes `rsp.first_tile_index` even when that tile is a stale definition tile rather than the loaded texture. On by default; this is the escape hatch for comparing. |
