...
neighbourhood, and it is not the same question as `geEnemiesNear`. In the test fixture
...
`vendor/ge-decomp` is the Mac build's lane. The port half is written, compiled and tested; the
