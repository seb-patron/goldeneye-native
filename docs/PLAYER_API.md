...
⚠️ **That lands in `vendor/ge-decomp/`, which is on the do-not-touch list and is gitignored**.
...
⚠️ **Do not make the agent read its own HUD.** Export the numbers. The MP HUD is additionally
