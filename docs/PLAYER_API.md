...
sections that depend on them say so inline.
...
**Unresolved and it matters.** Two mechanisms decouple simulation from rendering and the
...
it also decides what `docs/FRAME_TIMING.md` can assume.
...
**That lands in `vendor/ge-decomp/`, which is on the do-not-touch list and is gitignored** --
it travels only through `getv/patches/0001-source.patch`, which is still being revised. That
patch landing is the one external dependency this work has; nothing else blocks it.
...
| 11 | Uninitialised locals read into logic: `stan.c:1425,1471`; `chr.c:3957-3968`; `chrprop.c:1377-1390` indexes `((u8*)g_Textures)[-8]` **on every shot** | Retail depended on deterministic RDRAM garbage. Do not "fix" blindly -- some of it is necessary. |
...
- **Co-op players do not move today.** Four explanations eliminated; what remains is MP-specific
  gating between the pad read and the movement apply. This is a known open item and it blocks
  any bot demo in co-op.
...
**Do not make the agent read its own HUD.** Export the numbers. The MP HUD is additionally
...
4. **One bot that walks to a pad and fires**, per the queue's instruction to design against a
   ...
   on it.
5. **Game-side accessors** for health/armour/weapon/ammo/angle/room. Depends on the
   `vendor/ge-decomp/` patch in §5 landing first.
...
1. **Cross-platform bit-exactness is not realistically achievable.** x87 vs SSE vs ARM, and
   ...
   game**, with `-fno-strict-aliasing` and `-fwrapv`. That undefined behaviour is necessary, not accidental;
...
That is a much smaller search than "co-op movement", and it is localised to `bondview2.c`.
Worth recording these two facts rather than re-deriving them later.
